...
rule freebayes:
    ...
    shell:
        ...
        "module load python/2.7.15 samtools vcflib/gcc/64/0.00.2019.07.10 && freebayes-parallel <(fasta_generate_regions.py {input.reference}.fai 100000) 30 -f {input.reference} --use-best-n-alleles 4 --min-base-quality 10 --min-alternate-fraction 0.2 --haplotype-length 0 --ploidy 2 --min-alternate-count 2 --bam {input.bam} | vcffilter -f 'QUAL > 20'| bgzip -c > {output}"
